...
export default function DevisPage() {
  ...
  const loadDevis = async () => {
    ...
    setError(null);
    try {
      const data = await fetchDevis({ codeSociete, sucNeg, limit });
      setDevis(data);
    } catch (e: unknown) {
      setError('Erreur lors du chargement des devis. Vérifiez que le backend est accessible.');
      console.error(e);
    } finally {
      setLoading(false);
    }
        ...
        <div className="max-w-screen-2xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">IPS — Système de Suivi des Négociations</p>
            </div>
          </div>
      ...
      <div className="max-w-screen-2xl mx-auto px-6 py-6 space-y-5">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          <div className="flex flex-wrap gap-3 items-end">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-gray-600">Code Société</label>
              <input
                type="text"
                value={codeSociete}
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-gray-600">Succursale</label>
              <input
                type="text"
                value={sucNeg}
            ...
            <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
              <label className="text-xs font-medium text-gray-600">Recherche rapide</label>
        ...
        {!loading && !error && (
          <p className="text-sm text-gray-500">
            Affichage de <span className="font-semibold text-gray-800">{filtered.length}</span> devis
